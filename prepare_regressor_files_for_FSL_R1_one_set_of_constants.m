...
%This is a new version after peer review, but in reality the script should
%not change: the "source files" are different due to different adjustments,
%but that's all.

%Keep in mind to be in the correct folder (in my case, R1) to run updated
%files.
...
%6) Difference in effort discounting (ED): time lockedo the task onset, modulated by EDchosen-EDunchosen
...
filename_s1=strcat(pwd,'\fmri\task_logs\',pre_or_post,'_sub_',num2str(subject),'_subjective_value_whole_sample_session_1.mat');
filename_s2=strcat(pwd,'\fmri\task_logs\',pre_or_post,'_sub_',num2str(subject),'_subjective_value_whole_sample_session_2.mat');
filename_s3=strcat(pwd,'\fmri\task_logs\',pre_or_post,'_sub_',num2str(subject),'_subjective_value_whole_sample_session_3.mat');
...
min_value_SV=min(pre_demeaned_task_SV);
demeaned_task_SV(:,3)=(2.*((pre_demeaned_task_SV-min_value_SV)./(max_value_SV-min_value_SV)))-1; %This normalizes data between -1 and 1
...
min_value_ED=min(pre_demeaned_task_ED);
demeaned_task_ED(:,3)=(2.*((pre_demeaned_task_ED-min_value_ED)./(max_value_ED-min_value_ED)))-1; %This normalizes data between -1 and 1
...
min_value_PD=min(pre_demeaned_task_PD);
demeaned_task_PD(:,3)=(2.*((pre_demeaned_task_PD-min_value_PD)./(max_value_PD-min_value_PD)))-1; %This normalizes data between -1 and 1
%demeaned_task_PD(:,3)=pre_demeaned_task_PD./max_value_PD; %This puts all the values relative to 1

%Saving txt files
session=num2str(sesion);

filename_task=strcat(pwd,'\fmri\GLM_regressors\sub_',subject,'_task_boxcar_sesion_',session,'_',pre_or_post,'.txt');
filename_control=strcat(pwd,'\fmri\GLM_regressors\sub_',subject,'_control_sesion_',session,'_',pre_or_post,'.txt');
filename_NRNE=strcat(pwd,'\fmri\GLM_regressors\sub_',subject,'_NRNE_sesion_',session,'_',pre_or_post,'.txt');
filename_NRME=strcat(pwd,'\fmri\GLM_regressors\sub_',subject,'_NRME_sesion_',session,'_',pre_or_post,'.txt');
filename_SV=strcat(pwd,'\fmri\GLM_regressors\sub_',subject,'_task_SV_sesion_',session,'_',pre_or_post,'.txt');
filename_ED=strcat(pwd,'\fmri\GLM_regressors\sub_',subject,'_task_ED_sesion_',session,'_',pre_or_post,'.txt');
filename_PD=strcat(pwd,'\fmri\GLM_regressors\sub_',subject,'_task_PD_sesion_',session,'_',pre_or_post,'.txt');
