%This is a new version after peer review (May 2019). Constants have
%changed, so we need to calculate again subjective values.
...
%adjustment in the case of effort discounting, and from exponential for risk.
...
%We are using hyperbolic for effort and exponential for risk
file_K_effort=strcat(pwd,'\out-of-scanner_tasks\effort\',pre_or_post,'_analyses\',pre_or_post,'_effort_hyperbolic_intervention.mat');
file_K_risk=strcat(pwd,'\out-of-scanner_tasks\risk\',pre_or_post,'_analyses\',pre_or_post,'_risk_exponential_intervention.mat');
matrix_effort=load(file_K_effort);
matrix_risk=load(file_K_risk);
K_effort=matrix_effort.all_subs_hyp(:,1);
K_risk=matrix_risk.all_subs_exp(:,1);

%Keep in mind that subject 15 did not perform risk-discounting task PRE.
%Thus, we are taking the c value from POST to calculate SV (etc)

if strcmp(pre_or_post,'PRE')==1
    dummy_file_risk=strcat(pwd,'\out-of-scanner_tasks\risk\POST_analyses\POST_risk_exponential_intervention.mat');
    dummy_K_risk=load(dummy_file_risk);
    K_risk(15,1)=dummy_K_risk.all_subs_exp(15,1);
end
...
for sub=1:length(subjects)
    ...
    P_DF(sub,:)=exp(-K_risk(sub).*odds_against);
