...
%Loading input files:
    if str2double(habit_or_control)==1 && str2double(pre_or_post)==1
        input_adjustments=strcat('PRE_risk_discounting_adjustment_intervention.mat');
        input_DF=strcat('PRE_risk_discounting_factors_intervention.mat');
        subjects=1:19;
    elseif str2double(habit_or_control)==1 && str2double(pre_or_post)==2
        input_adjustments=strcat('POST_risk_discounting_adjustment_intervention.mat');
        input_DF=strcat('POST_risk_discounting_factors_intervention.mat');
        subjects=1:19;
    elseif str2double(habit_or_control)==2 && str2double(pre_or_post)==1
        input_adjustments=strcat('PRE_risk_discounting_adjustment_control.mat');
        input_DF=strcat('PRE_risk_discounting_factors_control.mat');
        subjects=1:13;
    elseif str2double(habit_or_control)==2 && str2double(pre_or_post)==2
        input_adjustments=strcat('POST_risk_discounting_adjustment_control.mat');
        input_DF=strcat('POST_risk_discounting_factors_control.mat');
        subjects=1:13;
    end
...
for subject=1:length(subjects)
    [I, J]=max(risk_discounting_adjustment(subject,2:5));
...
for zz=1:6
    %DF(zz,1)=nanmean(risk_discounting_factors([1:14,16:19],zz));
    %DF_SEM(zz,1)=nansem(risk_discounting_factors([1:14,16:19],zz));
    DF(zz,1)=nanmean(risk_discounting_factors(:,zz));
    DF_SEM(zz,1)=nansem(risk_discounting_factors(:,zz));    
end
...
xx=(1:100)'; %This is to show smooth fittings
...
group_risk_discounting_adjustment(1,1)=best_R2_hyperbolic;
group_risk_discounting_constants(1,1)=final_k;
...
group_risk_discounting_adjustment(1,2)=best_R2_exponential;
group_risk_discounting_constants(1,2)=final_c;
...
group_risk_discounting_adjustment(1,3)=best_R2_double;
group_risk_discounting_constants(1,3)=final_beta;
group_risk_discounting_constants(1,4)=final_delta;
...
group_risk_discounting_adjustment(1,4)=best_R2_parabolic;
group_risk_discounting_constants(1,5)=final_kpar;
group_risk_discounting_constants(1,6)=final_apar;
...
%plot(risk_levels,DF,'o','MarkerFaceColor','k','MarkerEdgeColor','k','MarkerSize',10) %Plot observed values without error bars
errorbar(risk_levels,DF,DF_SEM,'o','MarkerSize',5,'MarkerEdgeColor','black','MarkerFaceColor','black','Color','black','LineWidth',3); %Plot observed values with error bars (SEM)
ylim([0 1]);
...
legend('Hyperbolic','Exponental','Double exp.','Parabolic','Observed data')
    if str2double(habit_or_control)==1 && str2double(pre_or_post)==1
        output_graph=strcat('PRE_intervention_group_risk_discounting.fig');
    elseif str2double(habit_or_control)==1 && str2double(pre_or_post)==2
        output_graph=strcat('POST_intervention_group_risk_discounting.fig');
    elseif str2double(habit_or_control)==2 && str2double(pre_or_post)==1
        output_graph=strcat('PRE_control_group_risk_discounting.fig');
    elseif str2double(habit_or_control)==2 && str2double(pre_or_post)==2
        output_graph=strcat('POST_control_group_risk_discounting.fig');
    end    
...
%Saving main output:
if str2double(habit_or_control)==1 && str2double(pre_or_post)==1
    save('PRE_best_adjustment_risk_intervention.mat','best_adjustment');
    save('PRE_intervention_group_risk_discounting_constants.mat','group_risk_discounting_constants');
    save('PRE_intervention_group_risk_discounting_adjustment.mat','group_risk_discounting_adjustment');
    
elseif str2double(habit_or_control)==1 && str2double(pre_or_post)==2
    save('POST_best_adjustment_risk_intervention.mat','best_adjustment');
    save('POST_intervention_group_risk_discounting_constants.mat','group_risk_discounting_constants');
    save('POST_intervention_group_risk_discounting_adjustment.mat','group_risk_discounting_adjustment');
    
elseif str2double(habit_or_control)==2 && str2double(pre_or_post)==1
    save('PRE_best_adjustment_risk_control.mat','best_adjustment');
    save('PRE_control_group_risk_discounting_constants.mat','group_risk_discounting_constants');
    save('PRE_control_group_risk_discounting_adjustment.mat','group_risk_discounting_adjustment');
    
elseif str2double(habit_or_control)==2 && str2double(pre_or_post)==2
    save('POST_best_adjustment_risk_control.mat','best_adjustment');
    save('POST_control_group_risk_discounting_constants.mat','group_risk_discounting_constants');
    save('POST_control_group_risk_discounting_adjustment.mat','group_risk_discounting_adjustment');
