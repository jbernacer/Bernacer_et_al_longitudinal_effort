...
%         input_array_POST=load(input_data_POST);
    ...
    probability(first_position:last_position,:)=str2double([input_array_PRE.all_sessions{1,2}; input_array_PRE.all_sessions{2,2}]);
...
%     
    ...
    ZZ(2,2:7)=sum_choice(6:6:36,1)'; %Across subjects, times chosen 50€ for each risk level
    ...
    x_pcolor=[0 10 25 50 67 90 95;0 10 25 50 67 90 95;0 10 25 50 67 90 95;0 10 25 50 67 90 95;0 10 25 50 67 90 95;0 10 25 50 67 90 95;0 10 25 50 67 90 95;0 10 25 50 67 90 95];
...
output_graph=strcat('PRE_risk_discounting_all_subjects_colormap.fig');
...
for subject=1:length(subjects)
    ...
    opts = optimset('MaxFunEvals',50000, 'MaxIter',10000,'TolFun',1e-12,'TolX',1e-12); %Options
    [x,fval] = fmincon(@loglikelihood_hyp_risk,x0,[],[],[],[],[0 0],[10 100],[],opts) %x=estimated constants; fval=best log-likelihood
    POST__all_subs_hyp(1,1)=x(1); %Hyperbolic K
...
output_graph=strcat('POST_risk_discounting_all_subjects_colormap.fig');
    
savefig(output_graph);
hold off
POST_sum_choice=sum_choice;
POST_average_BIC(:,1)=mean(POST_LL_all_subs(:,2));
POST_average_BIC(:,2)=mean(POST_LL_all_subs(:,4));
POST_average_BIC(:,3)=mean(POST_LL_all_subs(:,6));
POST_average_BIC(:,4)=mean(POST_LL_all_subs(:,8));
...
save('PRE_vs_POST_risk_hyperbolic_all_subjects.mat','PREPOST_hyp');
save('PRE_vs_POST_risk_exponential_all_subjects.mat','PREPOST_exp');
save('PRE_vs_POST_risk_double_exp_all_subjects.mat','PREPOST_Dexp');
save('PRE_vs_POST_risk_parabolic_all_subjects.mat','PREPOST_par');
save('PRE_vs_POST_risk_BIC_all_subjects.mat','PREPOST_BIC');
save('PRE_vs_POST_risk_sum_alternative_chosen.mat','PREPOST_sum_choice');
...
ylim([0 1]);
output_graph=strcat('PRE_vs_POST_risk_discounting_hyperbolic.fig');
savefig(output_graph);
hold off
close(hyperbolic)
...
ylim([0 1]);
output_graph=strcat('PRE_vs_POST_risk_discounting_exponential.fig');
savefig(output_graph);
hold off
close(exponential)
...
ylim([0 1]);
output_graph=strcat('PRE_vs_POST_risk_discounting_double_exp.fig');
savefig(output_graph);
hold off
close(double_exp)
...
ylim([0 1]);
output_graph=strcat('PRE_vs_POST_risk_discounting_parabolic.fig');
savefig(output_graph);
hold off
close(parabolic)
