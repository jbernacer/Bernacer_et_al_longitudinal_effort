...
%Do you want to run this for the control or intervention group?:
habit_or_control=input('Analyze intervention (1) or control (2) group?    ','s');

%Do you want ro run this for the first or second evaluation?:
pre_or_post=input('Analyze pre (1) or post (2) data?   ','s');

%Control group=13 subjects; intervention group=19 subjects
...
for subject=1:length(subjects)
    ...
    all_subs_hyp(subject,1)=x(1); %Hyperbolic K
    all_subs_hyp(subject,2)=x(2); %Beta (inverse temperature)
    all_subs_hyp(subject,3)=fval; %LL
    LL_all_subs(subject,1)=-fval; %LL
    LL_all_subs(subject,2)=-2*LL_all_subs(subject,1)+2*log(144); %Bayesian Information Criterion (BIC)
    
    y0 = [0.1,1];
    opts = optimset('MaxFunEvals',50000, 'MaxIter',10000,'TolFun',1e-12,'TolX',1e-12); 
    [y,fval] = fmincon(@loglikelihood_exp,y0,[],[],[],[],[0 0],[10 100],[],opts)
    all_subs_exp(subject,1)=y(1); %Exponential c
    all_subs_exp(subject,2)=y(2); %Beta
    all_subs_exp(subject,3)=fval;
    LL_all_subs(subject,3)=-fval;
    LL_all_subs(subject,4)=-2*LL_all_subs(subject,3)+2*log(144);
    
    z0 = [3,0.1,1];
    opts = optimset('MaxFunEvals',50000, 'MaxIter',10000,'TolFun',1e-12,'TolX',1e-12); 
    [z,fval] = fmincon(@loglikelihood_Dexp,z0,[],[],[],[],[0 0 0],[10 50 100],[],opts)
    all_subs_Dexp(subject,1)=z(1); %Double exponential beta
    all_subs_Dexp(subject,2)=z(2); %Double exponential delta
    all_subs_Dexp(subject,3)=z(3); %Inverse temperature (beta)
    all_subs_Dexp(subject,4)=fval;
    LL_all_subs(subject,5)=-fval;
    LL_all_subs(subject,6)=-2*LL_all_subs(subject,5)+3*log(144);
    
    w0 = [0.000001,1];
    opts = optimset('MaxFunEvals',50000, 'MaxIter',10000,'TolFun',1e-12,'TolX',1e-12); 
    [w,fval] = fmincon(@loglikelihood_par,w0,[],[],[],[],[0 0],[10 100],[],opts)
    all_subs_par(subject,1)=w(1); %Parabolic K (NOTE THAT a IS FIXED TO 1, because otherwise curves are not realistic!)
    all_subs_par(subject,2)=w(2); %Beta
    all_subs_par(subject,3)=fval;
    LL_all_subs(subject,7)=-fval;
    LL_all_subs(subject,8)=-2*LL_all_subs(subject,7)+2*log(144);
    
    %What is the best curve for this subject?
    
    [I, J]=min(LL_all_subs(subject,[2,4,6,8]));
    if J==1
        best_curve{subject,1}='Hyperbolic';
    elseif J==2
        best_curve{subject,1}='Exponential';
    elseif J==3
        best_curve{subject,1}='Double exp.';
    elseif J==4
        best_curve{subject,1}='Parabolic';
    end
...
% and as close as possible to the "indifference points" (2 times).
    ...
    for k=1:36
        ...
        if sum_choice(k)==0 %If the effortful option was chosen 0 out of 4 times...
            %shape='.';
            %color=[0 0.2 0];
            txt='0';
        elseif sum_choice(k)==1 %If the effortful option was chosen 1 out of 4 times...
            %shape='*';
            %color=[0 0.4 0];
            txt='1';
        elseif sum_choice(k)==2 %If the effortful option was chosen 2 out of 4 times...
            %shape='+';
            %color=[0 0.6 0];
            txt='2';
        elseif sum_choice(k)==3 %If the effortful option was chosen 3 out of 4 times...
            %shape='o';
            %color=[0 0.8 0];
            txt='3';
        elseif sum_choice(k)==4 %If the effortful option was chosen 4 out of 4 times...
            %shape='p';
            %color=[0 1 0];
            txt='4';
        end
...
%Saving main outputs:
if str2double(habit_or_control)==1 && str2double(pre_or_post)==1
    save('PRE_effort_hyperbolic_intervention.mat','all_subs_hyp'); %K, inverse temperature, LL
    save('PRE_effort_exponential_intervention.mat','all_subs_exp'); %c, inverse temperature, LL
    save('PRE_effort_double_exp_intervention.mat','all_subs_Dexp'); %beta, delta, inverse temperature, LL
    save('PRE_effort_parabolic_intervention.mat','all_subs_par'); %k, inverse temperature, LL
    save('PRE_effort_average_BIC_intervention.mat','average_BIC'); %For hyperbolic, exponential, double exponential, parabolic
    save('PRE_effort_individual_best_curve_intervention.mat','best_curve');
elseif str2double(habit_or_control)==1 && str2double(pre_or_post)==2
    save('POST_effort_hyperbolic_intervention.mat','all_subs_hyp'); %K, inverse temperature, LL
    save('POST_effort_exponential_intervention.mat','all_subs_exp'); %c, inverse temperature, LL
    save('POST_effort_double_exp_intervention.mat','all_subs_Dexp'); %beta, delta, inverse temperature, LL
    save('POST_effort_parabolic_intervention.mat','all_subs_par'); %k, inverse temperature, LL
    save('POST_effort_average_BIC_intervention.mat','average_BIC'); %For hyperbolic, exponential, double exponential, parabolic
    save('POST_effort_individual_best_curve_intervention.mat','best_curve');
elseif str2double(habit_or_control)==2 && str2double(pre_or_post)==1
    save('PRE_effort_hyperbolic_control.mat','all_subs_hyp'); %K, inverse temperature, LL
    save('PRE_effort_exponential_control.mat','all_subs_exp'); %c, inverse temperature, LL
    save('PRE_effort_double_exp_control.mat','all_subs_Dexp'); %beta, delta, inverse temperature, LL
    save('PRE_effort_parabolic_control.mat','all_subs_par'); %k, inverse temperature, LL
    save('PRE_effort_average_BIC_control.mat','average_BIC'); %For hyperbolic, exponential, double exponential, parabolic
    save('PRE_effort_individual_best_curve_control.mat','best_curve');
elseif str2double(habit_or_control)==2 && str2double(pre_or_post)==2
    save('POST_effort_hyperbolic_control.mat','all_subs_hyp'); %K, inverse temperature, LL
    save('POST_effort_exponential_control.mat','all_subs_exp'); %c, inverse temperature, LL
    save('POST_effort_double_exp_control.mat','all_subs_Dexp'); %beta, delta, inverse temperature, LL
    save('POST_effort_parabolic_control.mat','all_subs_par'); %k, inverse temperature, LL
    save('POST_effort_average_BIC_control.mat','average_BIC'); %For hyperbolic, exponential, double exponential, parabolic
    save('POST_effort_individual_best_curve_control.mat','best_curve');
end   
