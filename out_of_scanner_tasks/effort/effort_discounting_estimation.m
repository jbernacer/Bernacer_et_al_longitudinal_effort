...
%option, for each effort level. By doing so, we can estimate the "indifference points"
%for each effort level. 3) A hyperbolic, exponential, double exponential or parabolic curve
%is used to fit the data.

%Main outputs:
%1) effort_indifference_points.mat: N-by-6 (N=subjects) matrix. Each column is the indifference point
%    for each effort level (5,10,15,20,25 or 30 min running)
%2) effort_psychometric_data.mat: N-by-M (N=subjects, M=effort levels) cell, with a 1-by-4 matrix in
%    each cell. This matrix shows K, G, r0 constants, and adjustment (R-squared)
%3) effort_discounting_constants.mat: N-by-7 (N=subjects) matrix, recording the
...
%4) effort_discounting_adjustment.mat: N-by-5 (N=subjects) matrix,
...
%THIS SCRIPT CAN BE USED FOR PRE (BEFORE HABIT) OR POST (AFTER HABIT) DATA, AND FOR "INTERVENTION" AND
%"CONTROL" GROUPS
...
effort_discounting_adjustment=zeros(length(subjects),5);
effort_discounting_constants=zeros(length(subjects),7);
psychometric_constants_and_adjustment=cell(length(subjects),6);
y_predicted_best=zeros(60,6);
all_efforts_y_observed=zeros(6,6);
...
for subject=1:length(subjects)
    ...
    if str2double(habit_or_control)==1 && str2double(pre_or_post)==1
        input_data=strcat('PRE_effort_discounting_sub',volunteer,'.mat');
        input_array=load(input_data);
    elseif str2double(habit_or_control)==1 && str2double(pre_or_post)==2
        input_data=strcat('POST_effort_discounting_sub',volunteer,'.mat');
        input_array=load(input_data);
    elseif str2double(habit_or_control)==2 && str2double(pre_or_post)==1
        input_data=strcat('PRE_effort_discounting_control',volunteer,'.mat');
        input_array=load(input_data);
    elseif str2double(habit_or_control)==2 && str2double(pre_or_post)==2
        input_data=strcat('POST_effort_discounting_control',volunteer,'.mat');
        input_array=load(input_data);
    end
    ...
    effort=str2double([input_array.all_sessions{1,2}; input_array.all_sessions{2,2}]);
    ...
    %effort level. The goal is to plot the probability of choosing the alternative option
    ...
    %choose the fixed (1) or the alternative (2) option for a particular effort level.
    ...
    %points are considered as Infinity (subject will not perform the effort
    %even though Inf € are offered) or as 5 (there is no discounting, and
    %the subject will make the effort even for the same amount of money
    ...
    %columns, being the indifference point for each effort level (5, 10, 15,
    %20, 25 and 30 minutes running)
        
    %Starting loop for each effort level:
    
    for e=1:length(effort_levels) %For each effort level
        
        effort=effort_levels(e);
        R2=cell(50,1,1);
        
        %y_observed are the actual probabilities of choosing the discounted option
        %for that effort level:
        ...
        if sum(y_observed==zeros(6,1))==6
            ...
            all_efforts_y_observed(:,e)=y_observed;
        ...
        %This will record each set of constants and adjustment per effort
        %level and subject
    ...
    h = figure;
    subplot(2,3,1)
    plot(money_amounts,all_efforts_y_observed(:,1),'o','MarkerFaceColor','b','MarkerSize',10)
    hold on
    ...
    subplot(2,3,2)
    plot(money_amounts,all_efforts_y_observed(:,2),'o','MarkerFaceColor','b','MarkerSize',10)
    hold on
    ...
    subplot(2,3,3)
    plot(money_amounts,all_efforts_y_observed(:,3),'o','MarkerFaceColor','b','MarkerSize',10)
    hold on
    ...
    subplot(2,3,4)
    plot(money_amounts,all_efforts_y_observed(:,4),'o','MarkerFaceColor','b','MarkerSize',10)
    hold on
    ...
    subplot(2,3,5)
    plot(money_amounts,all_efforts_y_observed(:,5),'o','MarkerFaceColor','b','MarkerSize',10)
    hold on
    ...
    subplot(2,3,6)
    plot(money_amounts,all_efforts_y_observed(:,6),'o','MarkerFaceColor','b','MarkerSize',10)
    hold on
    ...
    title('Effort = 30 min')
    if str2double(habit_or_control)==1 && str2double(pre_or_post)==1
        output_graph=strcat('PRE_psychometric_sub',volunteer,'_effort.fig');
    elseif str2double(habit_or_control)==1 && str2double(pre_or_post)==2
        output_graph=strcat('POST_psychometric_sub',volunteer,'_effort.fig');
    elseif str2double(habit_or_control)==2 && str2double(pre_or_post)==1
        output_graph=strcat('PRE_psychometric_control',volunteer,'_effort.fig');
    elseif str2double(habit_or_control)==2 && str2double(pre_or_post)==2
        output_graph=strcat('POST_psychometric_control',volunteer,'_effort.fig');
    end   
    ...
    %of each effort level is calculated as: X/ip, being X the effort level
    %and ip the indifference point. Then, different curves are fitted to
...
effort_discounting_adjustment(subject,1)=subject;
effort_discounting_adjustment(subject,2)=best_R2_hyperbolic;
effort_discounting_constants(subject,1)=subject;
effort_discounting_constants(subject,2)=final_k;
...
effort_discounting_adjustment(subject,3)=best_R2_exponential;
effort_discounting_constants(subject,3)=final_c;
...
effort_discounting_adjustment(subject,4)=best_R2_double;
effort_discounting_constants(subject,4)=final_beta;
effort_discounting_constants(subject,5)=final_delta;
...
effort_discounting_adjustment(subject,5)=best_R2_parabolic;
effort_discounting_constants(subject,6)=final_kpar;
effort_discounting_constants(subject,7)=final_apar;
...
% plot(effort_levels,DF,'o','MarkerFaceColor','k','MarkerEdgeColor','k','MarkerSize',10) %Plot observed values
% ylim([0 1]);
...
%     if str2double(habit_or_control)==1 && str2double(pre_or_post)==1
%         output_graph=strcat('PRE_effort_discounting_sub',volunteer,'.fig');
%     elseif str2double(habit_or_control)==1 && str2double(pre_or_post)==2
%         output_graph=strcat('POST_effort_discounting_sub',volunteer,'.fig');
%     elseif str2double(habit_or_control)==2 && str2double(pre_or_post)==1
%         output_graph=strcat('PRE_effort_discounting_control',volunteer,'.fig');
%     elseif str2double(habit_or_control)==2 && str2double(pre_or_post)==2
%         output_graph=strcat('POST_effort_discounting_control',volunteer,'.fig');
%     end    
% savefig(output_graph);
% hold off
% close(h)
...
% p5=plot(effort_levels,DF,'o','MarkerFaceColor','k','MarkerEdgeColor','k','MarkerSize',5); %Plot observed values
...
% title(title_text)
%     if str2double(habit_or_control)==1 && str2double(pre_or_post)==1
%         output_graph=strcat('PRE_effort_discounting_intervention_subjects.fig');
%     elseif str2double(habit_or_control)==1 && str2double(pre_or_post)==2
%         output_graph=strcat('POST_effort_discounting_intervention_subjects.fig');
%     elseif str2double(habit_or_control)==2 && str2double(pre_or_post)==1
%         output_graph=strcat('PRE_effort_discounting_control_subjects.fig');
%     elseif str2double(habit_or_control)==2 && str2double(pre_or_post)==2
%         output_graph=strcat('POST_effort_discounting_control_subjects.fig');
%     end   
...
effort_discounting_factors=5./indifference_points;
%Saving main output:
if str2double(habit_or_control)==1 && str2double(pre_or_post)==1
    save('PRE_effort_psychometric_data_intervention.mat','psychometric_constants_and_adjustment');
    save('PRE_effort_indifference_points_intervention.mat','indifference_points');
    save('PRE_effort_discounting_adjustment_intervention.mat','effort_discounting_adjustment');
    save('PRE_effort_discounting_constants_intervention.mat','effort_discounting_constants');
    save('PRE_effort_discounting_factors_intervention.mat','effort_discounting_factors');
elseif str2double(habit_or_control)==1 && str2double(pre_or_post)==2
    save('POST_effort_psychometric_data_intervention.mat','psychometric_constants_and_adjustment');
    save('POST_effort_indifference_points_intervention.mat','indifference_points');
    save('POST_effort_discounting_adjustment_intervention.mat','effort_discounting_adjustment');
    save('POST_effort_discounting_constants_intervention.mat','effort_discounting_constants');
    save('POST_effort_discounting_factors_intervention.mat','effort_discounting_factors');
elseif str2double(habit_or_control)==2 && str2double(pre_or_post)==1
    save('PRE_effort_psychometric_data_control.mat','psychometric_constants_and_adjustment');
    save('PRE_effort_indifference_points_control.mat','indifference_points');
    save('PRE_effort_discounting_adjustment_control.mat','effort_discounting_adjustment');
    save('PRE_effort_discounting_constants_control.mat','effort_discounting_constants');
    save('PRE_effort_discounting_factors_control.mat','effort_discounting_factors');
elseif str2double(habit_or_control)==2 && str2double(pre_or_post)==2
    save('POST_effort_psychometric_data_control.mat','psychometric_constants_and_adjustment');
    save('POST_effort_indifference_points_control.mat','indifference_points');
    save('POST_effort_discounting_adjustment_control.mat','effort_discounting_adjustment');
    save('POST_effort_discounting_constants_control.mat','effort_discounting_constants');
    save('POST_effort_discounting_factors_control.mat','effort_discounting_factors');
end   
toc